...
export default function ServicePage({
  label, title, subtitle, heroImg, color, intro, features, steps, industries, badge,
}: ServicePageProps) {
  return (
    <>
      <PageHero
      ...
      <div className="relative h-[50vh] overflow-hidden">
        ...
        {badge && (
          <div className="absolute top-6 right-6 bg-[#F5A623] text-white text-xs font-black uppercase tracking-widest px-4 py-2 rounded-full">
            {badge}
          </div>
        )}
      </div>
        ...
        <section className="py-16 bg-[#F8FAFF]">
          <div className="max-w-5xl mx-auto px-6 text-center">
            ...
            <div className="flex flex-wrap justify-center gap-3 mt-6">
              {industries.map(ind => (
                <span key={ind} className="px-5 py-2.5 bg-white border border-[#E8EDF5] rounded-full text-sm font-semibold text-[#1A1A2E] hover:border-[#003B8E]/25 hover:shadow-sm transition-all">
                  {ind}
                </span>
              ))}
            </div>
